...
"""
    @typechecked does not work when I do it on the constructor init method
    Try it and run "python main.py" to see the error...
         Its receiving int64 instead of int for the item param in the constructor
"""

class Item:
    
    def __init__(self, row: int, col: int, item: int):
        self.item: int = item
        self.item_name: str = reverse_id_map[item].replace("_", " ").title()
        self.row: int = row
        self.col: int = col
        self.calc_position()
